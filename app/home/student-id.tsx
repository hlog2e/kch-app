...
export default function StudentIDScreen() {
  ...
  const { data: userData } = useQuery({
    queryKey: ["IdCardUserData"],
    queryFn: getUserInfo,
  });

  const { mutate: registerBarcodeMutate } = useMutation({
    mutationFn: postRegisterBarCode,
  });
  ...
  const handleBarCodeScanned = async ({ data }: BarcodeScanningResult) => {
    ...
    registerBarcodeMutate(data, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["IdCardUserData"] });
  ...
  const age = Number(moment().format("YYYY")) - user!.birthYear + 1;
...
function Photo({ userData }: PhotoProps) {
  ...
  const handlePOSTRegisterPhoto = async (
...
function BarCodeSection({ userData, openBarCodeScanner }: BarCodeSectionProps) {
  const { colors } = useTheme();
  const styles = StyleSheet.create({
